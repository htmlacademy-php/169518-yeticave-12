    ...
    <div class="lot-item__content">
        <div class="lot-item__left">
            <div class="lot-item__image">
                <img src="<?= htmlspecialchars($single_item['image']); ?>" width="730" height="548"
                     alt="<?= htmlspecialchars($single_item['heading']); ?>">
            </div>
            ...
            <div class="lot-item__state">
                ...
                <div class="lot-item__cost-state">
                    <div class="lot-item__rate">
                        <span
                            class="lot-item__amount"><?= ($single_item['diff'] > '00:00:00') ? 'Текущая цена' : 'Выигравшая ставка' ?></span>
                        <span class="lot-item__cost"><?= htmlspecialchars($single_item['price']); ?> ₽</span>
                    </div>
                    <div
                        class="lot-item__min-cost" <?= ($single_item['diff'] > '00:00:00') ? '' : 'style="display:none;"' ?>>
                        Мин. ставка <span><?= htmlspecialchars($single_item['min_bet']); ?></span>
                    </div>
                </div>
                <?php if ($user && $single_item['diff'] > '00:00:00') : ?>
                    <form class="lot-item__form" action="lot.php?id=<?= htmlspecialchars($single_item['id']); ?>"
                          method="post" autocomplete="off">
                        <?php if (htmlspecialchars($user['id']) === htmlspecialchars($single_item['user_id'])) : ?>
                            <p>Вы не можете делать ставки на свой лот</p>
                        <?php elseif ($bets && (htmlspecialchars($user['id']) === htmlspecialchars($bets[0]['bet_user_id']))) : ?>
                        ...
                        <?php else : ?>
                            <p class="lot-item__form-item form__item <?= isset($errors['new-bet']) ? "form__item--invalid" : "" ?>">
                                ...
                                <input id="new-bet" type="text" name="new-bet"
                                       placeholder="<?= htmlspecialchars($single_item['min_bet']); ?>">
